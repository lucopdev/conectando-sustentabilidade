...
export async function POST(request: Request) {
  try {
    ...
    console.log('🔑 RESEND_API_KEY início:', process.env.RESEND_API_KEY?.substring(0, 5))
    ...
    if (!subject || !content) {
      ...
      return NextResponse.json({ message: 'Assunto e conteúdo são obrigatórios' }, { status: 400 });
    }
    ...
    if (!subscribers?.length) {
      ...
      return NextResponse.json({ message: 'Nenhum inscrito encontrado' }, { status: 404 });
    }
    ...
    return NextResponse.json({ message: 'Erro ao enviar newsletter', error }, { status: 500 });
  }
}
